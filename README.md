...
### 4.2 Pin Assignment (Arduino UNO R3)
...
The bench wiring is documented in two complementary forms, both under `hardware/schematic/`:

| Artefact | Format | Maintained by |
|---|---|---|
| `smart-gateway.sch` | EAGLE 7.3.0 XML | Hand-edited in EAGLE — net-level interconnect, the normative reference |
| `smart-gateway-breadboard.svg` / `.png` | SVG / PNG | Generated by `svggen.py` — physical placement and jumper colours |

![Smart Gateway breadboard view](hardware/schematic/smart-gateway-breadboard.png)

Regenerate the breadboard figure after any change to the pin assignment in §4.2:

```bash
sh hardware/schematic/schematic.sh
```

> [!NOTE]
> `svggen.py` registers every pin and rail, then asserts that both ends of every jumper land on one of them. A wire routed into empty space aborts generation rather than producing a plausible-looking but wrong diagram.

> [!IMPORTANT]
> `smart-gateway.sch` is a **schematic-only** sheet: its devices deliberately carry no package, so the file documents connectivity and will not generate a board layout. The keypad `C4` column and the RC522 `IRQ` line are intentionally left unconnected — see the pin-budget note in §4.2.

---

## 5. Firmware Architecture

### 5.1 Enforcement Node (Arduino UNO)
